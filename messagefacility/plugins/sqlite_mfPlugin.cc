...
namespace {
  ...
  private:
    void
    fillPrefix(std::ostringstream&, mf::ErrorObj const&) override
    {}
    void
    fillSuffix(std::ostringstream&, mf::ErrorObj const&) override
    {}
  ...
  //===============================================================================================================
  void
  sqlite3Plugin::routePayload(std::ostringstream const& oss,
                              mf::ErrorObj const& msg)
  {
    ...
    string const& iterationNo =
      mf::MessageDrop::instance()->iteration; // run/event no
    string const& modname = xid.module();     // module name
    string const& usrMsg = !oss.str().compare(0, 1, "\n") ?
                             oss.str().erase(0, 1) :
                             oss.str(); // user-supplied msg
                                        // (remove leading "\n" if present)
